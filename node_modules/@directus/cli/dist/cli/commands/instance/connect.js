...
exports.default = (0, command_1.command)({
    ...
    if (params.name in config.system.data.instances) {
        if (!params.force) {
            throw new exceptions_1.CLIRuntimeError(`Instance "${params.name}" already connected`);
        }
    }
    const sdk = new sdk_1.Directus(params.url);
    const instance = {
        auth: 'public',
        endpoint: params.url,
        data: {},
    ...
    if (params.public) {
        ...
        throw new exceptions_1.CLIRuntimeError('Unspecified authentication method.');
    }
    config.system.data.instances[params.name] = instance;
    config.system.save();
    await output.compose(async (ui) => {
        await ui.wrap((builder) => builder.line(`Successfully connect to ${chalk_1.default.bold(params.name)} through ${chalk_1.default.bold(instance.auth)} authentication`), 1);
    });
    return {
        success: true,
        mode: instance.auth,
