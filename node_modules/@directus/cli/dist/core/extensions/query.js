...
exports.default = (toolbox) => {
    ...
    toolbox.events.on('command.execute.before', async (command, options) => {
        ...
        if (one.fields) {
            one.fields = Array.from(new Set(one.fields.flatMap((field) => {
                return field.split(',').map((name) => name.trim());
            })));
        }
        const many = {
            ...one,
        ...
        if (many.sort) {
            many.sort = many.sort.flatMap((field) => {
                return field.split(',').map((name) => {
                    name = name.trim();
